...
// Redirect non-admins
if (!isAdmin) {
	await navigateTo(`/dashboard/series/${serieId.value}`)
}
	...
	<div class="edit-page flex flex-col flex-1 min-h-0">
		<UDashboardPanel class="flex-1 min-h-0">
			<template #header>
				<UiPageHeader
					:items="[
						{ label: 'Series', to: '/dashboard/series' },
						{ label: title, to: `/dashboard/series/${serieId}` },
						{ label: 'Edit' },
					]"
					:back-to="`/dashboard/series/${serieId}`"
				>
					<template #right>
						<button
